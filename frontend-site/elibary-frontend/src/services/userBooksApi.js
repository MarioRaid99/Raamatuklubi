...
export async function addMyBook(payload) {
  ...
  if (!res.ok) throw new Error(`POST /me/books failed: ${res.status}`);
  return await res.json();
}
...
export async function updateMyBook(bookId, payload) {
  ...
  if (!res.ok) throw new Error(`PATCH /me/books/${bookId} failed: ${res.status}`);
  return await res.json();
}

export async function removeMyBook(bookId) {
  const res = await apiFetch(`/me/books/${bookId}`, { method: "DELETE" });
  if (!res.ok && res.status !== 204) throw new Error(`DELETE /me/books/${bookId} failed: ${res.status}`);
  return true;
}
